...
def main
  patterns = find_patterns(15)

  combinators = {}
  patterns.each_pair {|k, v| 
    combinators[k] = v.map {|p| combinators_by_pattern(p) }.flatten.sort
  }
...
def find_patterns(max_n)
  ...
  patterns[1] = ["X"] # placeholder for variables
  ...
  patterns.each_pair {|k, v| 
    patterns[k] = v.keep_if {|p| p.start_with? "λ" }.sort
  }
end
